...
const VIA_REGION_CONTROL_POINT_SIZE = 2;
const VIA_REGION_POINT_RADIUS = 3;
const VIA_POLYGON_VERTEX_MATCH_TOL = 5;
const VIA_REGION_MIN_DIM = 3;
const VIA_MOUSE_CLICK_TOL = 2;
const VIA_ELLIPSE_EDGE_TOL = 0.2; // euclidean distance
const VIA_THETA_TOL = Math.PI / 18; // 10 degrees
const VIA_POLYGON_RESIZE_VERTEX_OFFSET = 100;
const VIA_CANVAS_DEFAULT_ZOOM_LEVEL_INDEX = 3;
const VIA_CANVAS_ZOOM_LEVELS = [0.25, 0.5, 0.75, 1.0, 1.5, 2.0, 2.5, 3.0, 4, 5];

const VIA_THEME_REGION_BOUNDARY_WIDTH = 4;
const VIA_THEME_BOUNDARY_LINE_COLOR = '#1a1a1a';
const VIA_THEME_BOUNDARY_FILL_COLOR = '#aaeeff';
const VIA_THEME_SEL_REGION_FILL_COLOR = '#808080';
const VIA_THEME_SEL_REGION_FILL_BOUNDARY_COLOR = '#000000';
const VIA_THEME_SEL_REGION_OPACITY = 0.5;
const VIA_THEME_MESSAGE_TIMEOUT_MS = 6000;
const VIA_THEME_ATTRIBUTE_VALUE_FONT = '10pt Sans';
const VIA_THEME_CONTROL_POINT_COLOR = '#ff0000';

const VIA_CSV_SEP = ',';
const VIA_CSV_QUOTE_CHAR = '"';
...
let _via_image_id_list = []; // array of image id (in original order)
...
// image canvas
let _via_img_canvas = document.getElementById('image_canvas');
let _via_img_ctx = _via_img_canvas.getContext('2d');
...
let _via_current_image_loaded = false;
let _via_is_window_resized = false;
let _via_is_user_resizing_region = false;
let _via_is_user_moving_region = false;
let _via_is_user_drawing_polygon = false;
let _via_is_region_selected = false;
let _via_is_all_region_selected = false;
let _via_is_user_updating_attribute_name = false;
let _via_is_user_updating_attribute_value = false;
let _via_is_user_adding_attribute_name = false;
let _via_is_loaded_img_list_visible = false;
let _via_is_attributes_panel_visible = false;
let _via_is_reg_attr_panel_visible = false;
let _via_is_file_attr_panel_visible = false;
...
// region
let _via_current_shape = VIA_REGION_SHAPE.RECT;
let _via_current_polygon_region_id = -1;
let _via_user_sel_region_id = -1;
let _via_click_x0 = 0;
let _via_click_y0 = 0;
let _via_click_x1 = 0;
let _via_click_y1 = 0;
let _via_region_click_x, _via_region_click_y;
let _via_copied_image_regions = [];
let _via_region_edge = [-1, -1];
let _via_current_x = 0;
let _via_current_y = 0;

// message
let _via_message_clear_timer;

// attributes
let _via_region_attributes = {};
let _via_current_update_attribute_name = '';
let _via_current_update_region_id = -1;
let _via_file_attributes = {};
let _via_visible_attr_name = '';

// persistence to local storage
let _via_is_local_storage_available = false;
let _via_is_save_ongoing = false;

// image list
let _via_reload_img_fn_list_table = true;
let _via_loaded_img_fn_list = [];
let _via_loaded_img_fn_list_file_index = [];
let _via_loaded_img_fn_list_table_html = [];
...
function show_image(image_index) {
    ...
    img_reader.addEventListener(
        ...
        function () {
            ...
            _via_current_image.addEventListener(
                'error',
                function () {
                    _via_is_loading_current_image = false;
                    img_loading_spinbar(false);
...
_via_reg_canvas.addEventListener('mouseup', function (e) {
    ...
    if (click_dx < VIA_MOUSE_CLICK_TOL || click_dy < VIA_MOUSE_CLICK_TOL) {
        ...
        if (_via_is_user_drawing_polygon) {
            ...
            if (
                Math.sqrt(dx * dx + dy * dy) <= VIA_POLYGON_VERTEX_MATCH_TOL &&
...
_via_reg_canvas.addEventListener('mousemove', function (e) {
    ...
    if (_via_is_user_resizing_region) {
        ...
        switch (attr['name']) {
            ...
            case VIA_REGION_SHAPE.ELLIPSE:
                ...
                _via_draw_ellipse_region(attr['cx'], attr['cy'], new_rx, new_ry, true);
                break;
            ...
            case VIA_REGION_SHAPE.POLYGON:
                ...
                _via_draw_polygon_region(moved_all_points_x, moved_all_points_y, true);
                break;
        }
        _via_reg_canvas.focus();
    }
    ...
    if (_via_is_user_moving_region) {
        ...
        switch (attr['name']) {
            ...
            case VIA_REGION_SHAPE.ELLIPSE:
                _via_draw_ellipse_region(attr['cx'] + move_x, attr['cy'] + move_y, attr['rx'], attr['ry'], true);
                break;
...
function _via_redraw_img_canvas() {
    if (_via_current_image_loaded) {
        ...
        _via_img_ctx.drawImage(_via_current_image, 0, 0, _via_img_canvas.width, _via_img_canvas.height);
    }
}
...
function draw_all_regions() {
    for (var i = 0; i < _via_canvas_regions.length; ++i) {
        ...
        switch (attr['name']) {
            ...
            case VIA_REGION_SHAPE.ELLIPSE:
                _via_draw_ellipse_region(attr['cx'], attr['cy'], attr['rx'], attr['ry'], is_selected);
                break;

            case VIA_REGION_SHAPE.POLYLINE: // handled by polygon
            case VIA_REGION_SHAPE.POLYGON:
                _via_draw_polygon_region(attr['all_points_x'], attr['all_points_y'], is_selected);
                break;
...
function _via_draw_ellipse_region(cx, cy, rx, ry, is_selected) {
    if (is_selected) {
        ...
        if (rx > VIA_THEME_REGION_BOUNDARY_WIDTH && ry > VIA_THEME_REGION_BOUNDARY_WIDTH) {
            ...
            _via_reg_ctx.strokeStyle = VIA_THEME_BOUNDARY_LINE_COLOR;
            _via_reg_ctx.lineWidth = VIA_THEME_REGION_BOUNDARY_WIDTH / 4;
            _via_draw_ellipse(cx, cy, rx + VIA_THEME_REGION_BOUNDARY_WIDTH / 2, ry + VIA_THEME_REGION_BOUNDARY_WIDTH / 2);
            _via_reg_ctx.stroke();
            _via_draw_ellipse(cx, cy, rx - VIA_THEME_REGION_BOUNDARY_WIDTH / 2, ry - VIA_THEME_REGION_BOUNDARY_WIDTH / 2);
            _via_reg_ctx.stroke();
        }
    ...
    if (is_selected) {
        ...
        for (var i = 1; i < all_points_x.length; ++i) {
            ...
            if (slope_i > 0) {
                ...
                _via_reg_ctx.beginPath();
                _via_reg_ctx.moveTo(
                    parseInt(all_points_x[i - 1]) - parseInt(VIA_THEME_REGION_BOUNDARY_WIDTH / 4),
                    parseInt(all_points_y[i - 1]) + parseInt(VIA_THEME_REGION_BOUNDARY_WIDTH / 4),
                );
                _via_reg_ctx.lineTo(
                    parseInt(all_points_x[i]) - parseInt(VIA_THEME_REGION_BOUNDARY_WIDTH / 4),
...
function is_inside_this_region(px, py, region_id) {
    ...
    switch (attr['name']) {
        ...
        case VIA_REGION_SHAPE.ELLIPSE:
            result = is_inside_ellipse(attr['cx'], attr['cy'], attr['rx'], attr['ry'], px, py);
            break;
...
function is_inside_polygon(all_points_x, all_points_y, px, py) {
    ...
    for (var i = 0; i < all_points_x.length - 1; ++i) {
...
function is_on_region_corner(px, py) {
    ...
    for (var i = 0; i < _via_canvas_regions.length; ++i) {
        ...
        switch (attr['name']) {
            ...
            case VIA_REGION_SHAPE.ELLIPSE:
                result = is_on_ellipse_edge(attr['cx'], attr['cy'], attr['rx'], attr['ry'], px, py);
                break;
...
_via_reg_canvas.addEventListener('keyup', function (e) {
    if (_via_is_user_updating_attribute_value || _via_is_user_updating_attribute_name || _via_is_user_adding_attribute_name) {
        return;
    }
...
_via_reg_canvas.addEventListener('keydown', function (e) {
    ...
    if (e.which === 27) {
        ...
        if (_via_is_user_updating_attribute_name || _via_is_user_updating_attribute_value) {
            _via_is_user_updating_attribute_name = false;
            _via_is_user_updating_attribute_value = false;
        }
...
function zoom_in() {
    ...
    if (_via_canvas_zoom_level_index === VIA_CANVAS_ZOOM_LEVELS.length - 1) {
        ...
        var zoom_scale = VIA_CANVAS_ZOOM_LEVELS[_via_canvas_zoom_level_index];
        set_all_canvas_scale(zoom_scale);
        set_all_canvas_size(_via_canvas_width * zoom_scale, _via_canvas_height * zoom_scale);
...
function save_current_data_to_browser_cache() {
    setTimeout(function () {
        if (_via_is_local_storage_available && !_via_is_save_ongoing) {
            try {
...
function is_via_data_in_localStorage() {
    return localStorage.getItem('_via_timestamp') && localStorage.getItem('_via_img_metadata');
}
...
function init_spreadsheet_input(type, col_headers, data, row_names) {
    ...
    firstrow.insertCell(-1).innerHTML =
        '<input type="text"' +
        ' onchange="add_new_attribute(\'' +
        type[0] +
        '\', this.value)"' +
        ' value = "[ Add New ]"' +
            ...
            if (di.hasOwnProperty(key)) {
                ...
                if (ip_val.length > 30) {
                    row.insertCell(-1).innerHTML =
                        ...
                        '"' +
                        ' autocomplete="on"' +
                        ' onchange="update_attribute_value(\'' +
                        input_id +
                        '\', this.value)"' +
                        ' onblur="attr_input_blur(' +
                        row_i +
                        ')"' +
                        ' onfocus="attr_input_focus(' +
                        row_i +
                        ');"' +
                        ' >' +
                        ip_val +
...
function update_region_attributes_input_panel() {
    init_spreadsheet_input('region_attributes', _via_region_attributes, _via_img_metadata[_via_image_id].regions);
}

function update_file_attributes_input_panel() {
    init_spreadsheet_input('file_attributes', _via_file_attributes, [_via_img_metadata[_via_image_id].file_attributes], [_via_current_image_filename]);
}
